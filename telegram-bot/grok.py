...
def ask_grok(user_message: str, history: list = None, memory_facts: list = None) -> str:
    """Чат с Grok. history — список {"role": ..., "content": ...}. memory_facts — долгосрочная память."""
    system = SYSTEM_PROMPT
    if memory_facts:
        facts_text = "\n".join(f"{i + 1}. {f}" for i, f in enumerate(memory_facts))
        system += f"\n\nДолгосрочная память (запомнено навсегда, используй как контекст):\n{facts_text}"
