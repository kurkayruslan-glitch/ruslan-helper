...
_DIALOG_ANALYSIS_SYSTEM = (
    ...
    "Контекст для всех аудиоразборов: это скрипт Днепровского офиса. Учитывай это при определении ролей, места, "
    ...
    "## Главные оценки 1-10\n\n"
    "В самом начале отчёта дай таблицу `| Критерий | Балл 1-10 | Что видно в диалоге | Почему такой балл | Как улучшить безопасно |`.\n"
    ...
    "и не просит лишнего.\n"
    "- Комплаенс / безопасность: проверяемость и легитимность звонка — имя, должность, организация, официальный номер, "
    "номер обращения, способ проверить, согласие, безопасный официальный канал.\n"
    "- Потенциал конверсии: шанс довести звонок до законного результата без разрушения доверия и без небезопасного сбора данных.\n\n"
    "## Что найдено в тексте\n\n"
    ...
    "Если встречаются карта, коды, СМС, Госуслуги, паспортные или банковские данные — не предлагай способы выманить их по телефону. "
    "Оцени риск и рекомендуй только безопасный официальный канал: офис, официальный сайт/приложение, подтверждённый номер или личная сверка.\n\n"
    "## Этап 1. Расшифровка\n\n"
    ...
    "`Как закрыть сопротивление без давления и небезопасного сбора данных`.\n\n"
    "## Этап 5.1. Оценка менеджера отдельно\n\n"
    "Если в диалоге есть менеджер/инфо-сборщик, дай таблицу `| Навык | Балл 1-10 | Доказательство из текста | Что улучшить безопасно |`.\n"
    ...
    "Перепиши разговор в виде идеального короткого диалога. Не меняй законную суть цели, но убери давление, обман, угрозы, "
    "обесценивание и хаос. Добавь признание опасений, объяснение регламента, варианты выбора, безопасный официальный канал "
    "и спокойную фиксацию решения. Диалог должен показывать, как инфо-сборщик максимально профессионально ведёт человека "
    "к предоставлению нужной информации или к официальной альтернативе, если данные нельзя взять по телефону.\n\n"
    "## Этап 9. Итог\n\n"
    "### Главные ошибки\n"
    "Нумерованный список.\n\n"
...
_DIALOG_ANALYSIS_KRYVEN_SYSTEM = (
    ...
    "Контекст: это скрипт Днепровского офиса. Оценивай звонок как работу менеджера по этому скрипту, "
    ...
    "## Главные оценки 1-10\n"
    "Таблица `| Критерий | Балл 1-10 | Что видно в диалоге | Почему такой балл | Как улучшить безопасно |`. "
    "Критерии: контроль разговора, доверие, ясность инструкции, движение к легитимной цели, "
    "комплаенс / безопасность, потенциал конверсии. "
    ...
    "давление (`необходимо`, `нужно`, `обязательно`, `иначе`, `срочно`, `должны`, `не получите`), "
    "проверяемость (`меня зовут`, `должность`, `организация`, `официальный номер`, `номер обращения`, `можете проверить`), "
    "следующий шаг. Если встречаются карта, коды, СМС, Госуслуги, паспортные или банковские данные — не предлагай способы "
    "получить их по телефону; пометь риск и предложи только официальный безопасный канал.\n\n"
    "## Этап 1. Расшифровка\n"
    ...
    "## Этап 5.1. Оценка менеджера отдельно\n"
    "Таблица `| Навык | Балл 1-10 | Доказательство из текста | Что улучшить безопасно |`. "
    "Навыки: уверенность, лидерство, контроль разговора, логика, аргументация, работа с возражениями, "
    "эмоциональный интеллект, умение слушать, умение задавать вопросы, влияние, настойчивость, гибкость, "
    "стрессоустойчивость, профессионализм. Оцени не только результат, а путь к нему: доверие, объяснение, структура, "
    "проверяемость, комплаенс.\n\n"
    "## Этап 6. Психология\n"
...
_DIALOG_CHUNK_SYSTEM = (
    "Ты готовишь рабочие заметки для финального разбора звонка. Контекст: это скрипт Днепровского офиса. "
    ...
    "комплаенс/безопасность, потенциал конверсии, давление, проверяемость и следующий шаг. "
    "Если встречаются СНИЛС, паспорт, адрес, реквизиты, карта, код, СМС или Госуслуги — пометь риск и не предлагай "
    "способы получить эти данные по телефону; только официальный безопасный канал. "
...
def _analyze_dialog_audio(chat_id: int, audio_bytes: bytes, filename: str, progress_message_id: int | None = None):
    ...
    try:
        ...
        _set_dialog_progress(
            ...
            "Передаю расшифровку в Kryven с контекстом: скрипт Днепровского офиса.",
        )
